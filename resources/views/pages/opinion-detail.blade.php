@extends('layouts.app')
@section('title', ($opinion->meta_title ?? $opinion->title) . ' — Nyay Vidhan')
...
@if ($opinion->og_image)
    @section('og_image', asset('storage/' . $opinion->og_image))
@endif
        ...
        <div class="container">
            <div class="page-layout">
                <article class="article-detail">
                    ...
                    <h1 class="article-title">{{ $opinion->title }}</h1>
                    ...
                    @if ($opinion->author)
                        <div class="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom flex-wrap">
                            ...
                            @else
                                <div
                                    style="width:52px;height:52px;border-radius:50%;background:#1a1a2e;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:1.1rem;flex-shrink:0">
                                    {{ strtoupper(substr($opinion->author->name, 0, 1)) }}
                                </div>
                            @endif
                            <div>
                                <div class="fw-semibold">
                                    <a href="{{ route('author.show', $opinion->author->slug) }}"
                                        class="text-decoration-none text-dark">{{ $opinion->author->name }}</a>
                                </div>
                    ...
                    @if ($opinion->excerpt)
                        <p class="article-excerpt text-muted"
                            style="font-size:1.05rem;margin-bottom:1.5rem;font-style:italic">{{ $opinion->excerpt }}</p>
                    @endif
                                ...
                                <article class="news-card">
                                    <div class="news-card-body">
                                        ...
                                        <h3 class="news-headline">
                                            <a href="{{ route('opinions.show', $r->slug) }}"
                                                class="text-inherit text-decoration-none">{{ $r->title }}</a>
                                        </h3>
