@extends('layouts.app')
@section('title', $q ? 'Search: '.e($q).' — TestLaw' : 'Search — TestLaw')
@section('meta_description', 'Search results for '.e($q).' on TestLaw.')
        ...
        <div class="page-layout">
                <div class="mt-4 mb-4">
                    <form method="GET" action="{{ route('search') }}" class="d-flex gap-2">
                        <input type="text" name="q" value="{{ e($q) }}"
                               ...
                               placeholder="Search judgments, articles, case names…"
                               autofocus />
                ...
                @if(strlen($q) >= 2)
                    <p class="text-muted mb-4">
                        @if($total > 0)
                            Found <strong>{{ $total }}</strong> result{{ $total !== 1 ? 's' : '' }} for <strong>"{{ e($q) }}"</strong>
                        @else
                    ...
                    <div class="border-bottom pb-3 mb-3">
                        ...
                        <h3 class="h5 mb-1">
                            <a href="{{ $result['url'] }}" class="text-decoration-none text-dark">{{ $result['title'] }}</a>
                        </h3>
                        @if($result['excerpt'])
                        ...
                        <div class="text-muted small d-flex gap-3">
                            ...
                            @if($result['published_at'])<span>{{ $result['published_at']->diffForHumans() }}</span>@endif
                        </div>
                    </div>
                    @empty
                    @endforelse
                ...
                @else
                    <div class="text-center py-5 text-muted">
                        <p class="mb-1">Search across all legal content — articles, judgments, interviews, opinions, and news.</p>
