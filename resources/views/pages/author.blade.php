@extends('layouts.app')
@section('title', $author->name . ' — Nyay Vidhan')
@section('meta_description', Str::limit($author->bio ?? 'Articles and content by ' . $author->name . ' on Nyay Vidhan.',
    160))
@section('og_title', $author->name . ' — Nyay Vidhan')
@section('og_description', Str::limit($author->bio ?? '', 160))
@if ($author->image)
    @section('og_image', asset('storage/' . $author->image))
@endif
    ...
    <main>
        <div class="container">
            ...
            <div class="page-layout">
                {{-- Author Bio Card --}}
                <div class="author-bio-card d-flex align-items-start gap-4 mb-5 mt-2 p-4"
                    ...
                    @else
                        <div
                            style="width:90px;height:90px;border-radius:50%;background:#1a1a2e;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:2rem;flex-shrink:0">
                    ...
                    <div>
                        ...
                        <div class="d-flex gap-3 mt-2">
                            @if ($author->twitter)
                                <a href="https://x.com/{{ ltrim($author->twitter, '@') }}" target="_blank" rel="noopener"
                            ...
                            @if ($author->email)
                                <a href="mailto:{{ $author->email }}"
                                    class="text-muted small text-decoration-none">{{ $author->email }}</a>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        @foreach ($articles as $article)
                            <article class="news-card">
                                ...
                                <div class="news-card-body">
                                    ...
                                    <div class="news-date text-muted small">{{ $article->published_at?->diffForHumans() }}
                                    </div>
                                </div>
                            </article>
                        @endforeach
                                ...
                                <div class="news-card-body">
                                    ...
                                    <h3 class="news-headline">
                                        <a href="{{ route('interviews.show', $interview->slug) }}"
                                            class="text-inherit text-decoration-none">{{ $interview->title }}</a>
                                    </h3>
                                    <div class="news-date text-muted small">
                                        {{ $interview->published_at?->diffForHumans() }}</div>
                                </div>
                            </article>
                        @endforeach
                    </div>
                        ...
                        @foreach ($opinions as $opinion)
                            <article class="news-card">
                                <div class="news-card-body">
                                    ...
                                    <div class="news-date text-muted small">{{ $opinion->published_at?->diffForHumans() }}
                                    </div>
                                </div>
                            </article>
                        @endforeach
