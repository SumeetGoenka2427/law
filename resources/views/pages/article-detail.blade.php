@extends('layouts.app')
@section('title', ($article->meta_title ?? $article->title) . ' — Nyay Vidhan')
...
@if ($article->og_image)
    @section('og_image', asset('storage/' . $article->og_image))
@endif
        ...
        <div class="container">
            <div class="page-layout">
                <article class="article-detail">
                        ...
                        @if ($article->author)
                            <span class="text-muted small">By
                                <a href="{{ route('author.show', $article->author->slug) }}"
                                    class="text-decoration-none text-muted fw-semibold">{{ $article->author->name }}</a>
                            </span>
                        @endif
                    ...
                    @if ($article->excerpt)
                        <p class="article-excerpt text-muted" style="font-size:1.05rem;margin-bottom:1.5rem">
                            {{ $article->excerpt }}</p>
                    @endif
                    @if ($article->image)
                        <div class="article-hero-img mb-4"><img src="{{ asset('storage/' . $article->image) }}"
                                alt="{{ $article->title }}" class="img-fluid rounded w-100" /></div>
                    @endif
                            ...
                            @foreach ($related as $r)
                                <article class="news-card">
                                    <div class="news-card-body">
                                        ...
                                        <h3 class="news-headline"><a href="{{ route('articles.show', $r->slug) }}"
                                                class="text-inherit text-decoration-none">{{ $r->title }}</a></h3>
                                        <div class="news-date text-muted small">{{ $r->published_at?->diffForHumans() }}
                                        </div>
                                    </div>
                                </article>
                            @endforeach
