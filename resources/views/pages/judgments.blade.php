@extends('layouts.app')
@section('title', 'Judgments — TestLaw')
            ...
            <div class="judgment-list">
                ...
                <div class="judgment-item">
                    ...
                    <div class="judgment-content">
                        ...
                        <div class="judgment-title"><a href="{{ route('judgments.show', $j->slug) }}" class="text-inherit text-decoration-none">{{ $j->title }}</a></div>
                        <div class="judgment-summary">{{ $j->excerpt }}</div>
                        @if($j->decided_at)<div class="judgment-date">Decided: {{ $j->decided_at->format('F j, Y') }}{{ $j->bench ? ' | '.$j->bench : '' }}</div>@endif
                    </div>
                </div>
                @empty
