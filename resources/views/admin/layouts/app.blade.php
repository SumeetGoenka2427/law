...
<head>
    ...
    <title>@yield('title', 'Admin') — TestLaw CMS</title>
    ...
    <style>
        ...
        .sidebar .nav-label { font-size: .65rem; font-weight: 600; letter-spacing: .08em; color: rgba(255,255,255,.4); padding: .75rem 1.25rem .25rem; text-transform: uppercase; }
        ...
        .badge-archived { background: #dc3545; }
        .table th { font-size: .8rem; font-weight: 600; letter-spacing: .03em; text-transform: uppercase; color: #6c757d; background: #f8f9fa; }
...
<div class="sidebar">
    ...
    <nav class="mt-2">
        ...
        <a href="{{ route('admin.judgments.index') }}" class="nav-link {{ request()->routeIs('admin.judgments*') ? 'active' : '' }}">
            <i class="bi bi-shield-check"></i> Judgments
        </a>
        <a href="{{ route('admin.latest-news.index') }}" class="nav-link {{ request()->routeIs('admin.latest-news*') ? 'active' : '' }}">
        ...
        <form method="POST" action="{{ route('admin.logout') }}">
            @csrf
            <button type="submit" class="nav-link btn btn-link p-0 text-start w-100" style="border:0;background:none;">
