@extends('layouts.app')
@section('title', $category->name . ' — Nyay Vidhan')
@section('meta_description', $category->description ?? 'Browse ' . $category->name . ' coverage on Nyay Vidhan.')
@section('og_title', $category->name . ' — Nyay Vidhan')
@section('og_description', $category->description ?? 'Browse ' . $category->name . ' coverage on Nyay Vidhan.')
        ...
        <div class="container">
            ...
            <div class="page-layout">
                <div class="d-flex align-items-center gap-2 mb-4">
                    @if ($category->color)
                        <span
                            style="width:6px;height:36px;border-radius:3px;background:{{ $category->color }};display:inline-block;flex-shrink:0"></span>
                    @endif
                            ...
                            <div class="judgment-item">
                                ...
                                <div class="judgment-content">
                                    ...
                                    <div class="judgment-title">
                                        <a href="{{ route('judgments.show', $j->slug) }}"
                                            class="text-inherit text-decoration-none">{{ $j->title }}</a>
                                    </div>
                                    @if ($j->decided_at)
                                        <div class="judgment-date">Decided: {{ $j->decided_at->format('F j, Y') }}</div>
                                    @endif
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="news-grid">
                        @foreach ($news as $item)
                            <article class="news-card">
                                <div class="news-card-body">
                                    @if ($item->is_breaking)
                                        <span class="tag" style="background:#dc3545">Breaking</span>
                                    @endif
                                    <h3 class="news-headline">
                                        <a href="{{ route('latest-news.show', $item->slug) }}"
                                            class="text-inherit text-decoration-none">{{ $item->title }}</a>
                                    </h3>
                                    <div class="news-date text-muted small">{{ $item->published_at?->diffForHumans() }}
                                    </div>
                                </div>
                            </article>
                        @endforeach
